...
export function getDayRangeForTimeZone(timeZone: string, referenceDate = new Date()) {
  const day = readDayParts(referenceDate, timeZone);
  const nextDay = addDays(day, 1);

  return {
    startIso: toUtcIso(day, timeZone),
    endIso: toUtcIso(nextDay, timeZone),
  };
}

export function getTodayRangeForTimeZone(timeZone: string, referenceDate = new Date()) {
  return getDayRangeForTimeZone(timeZone, referenceDate);
}
